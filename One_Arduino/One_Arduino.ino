...
void follow_line() {
  ...
  if (old_distance <= 15) { // close in on box now.
    readLight = true;
    readSecond = true;
    askForDist = true;
...
void close_in() {             // closing in on the box
  ...
  follow_line_sub(true);

  speed = old_distance <= 6 ? SPEED_VSLOW : SPEED_SLOW;


  close_enough = (old_distance <= 3 && wasBall) || (isBall && !wasBall);
